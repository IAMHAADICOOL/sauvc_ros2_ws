...
def main():
    ...
    while True:
        ...
        if out is None:
            continue
        vx, vy = out['vx'], out['vy']
        # FIX(reference-hold compat): after flow_core holds its reference across a
        # failed-tracking gap, the recovered velocity spans dt_eff (gap included).
        # Integrating it over the single-frame dt would silently drop the held time
        # and shrink the path. Fall back to dt for older flow_core versions.
        step_dt = float(out.get('dt_eff', dt))
        if args.yaw_rate:
            yaw += wz * dt
        # integrate in a world frame rotated by yaw (yaw=0 if no gyro given)
        x += (vx * math.cos(yaw) - vy * math.sin(yaw)) * step_dt
        y += (vx * math.sin(yaw) + vy * math.cos(yaw)) * step_dt
