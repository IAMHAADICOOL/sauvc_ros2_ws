...
class FlowEvalNode(Node):
    ...
    def _fused_yaws(self, t_wall):
        ...
        if (self.use_lane and self.lane_yaw is not None
            ...
            yaw_ned = self.lane_yaw + self.pool_axis_ned
    ...
    def on_image(self, msg):
        ...
        if res is None:
            ...
            self.ekf.update_flow(vx_b, vy_b, yaw_cmp, t, r_var=r_var)
            fx_, fy_ = self.flow_pos.update(vx_b, vy_b, yaw_cmp, t)
            ...
            if self.gtsam.available:
                ...
                # (vx_b/vy_b already carry the ZUPT clamp; reuse the fused NED yaw.)
                c, sn = np.cos(yaw_ned_used), np.sin(yaw_ned_used)
