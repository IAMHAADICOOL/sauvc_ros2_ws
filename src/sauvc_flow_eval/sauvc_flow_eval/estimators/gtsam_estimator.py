...
class GtsamEstimator:
    ...
    def __init__(self, gravity=9.81, accel_sigma=0.02, gyro_sigma=0.0017,
        ...
        # FIX(periodic spikes + frame drops): ISAM2's DEFAULT relinearizeSkip=10 batches
        # relinearization into one heavy solve every N keyframes. Measured in the eval
        # log: /eval/gtsam jump-out-and-back pairs every 14 print intervals (2.8 s) AND
        # flow freezes at EXACTLY the same 14-interval period — one periodic stall in
        # the single callback thread causing both. Relinearize a little every update
        # (skip=1, tighter threshold) instead of a lot occasionally: the per-update
        # cost rises slightly but the 2.8 s spike train disappears, which is what was
        # dropping camera frames. API differs across gtsam versions, hence the guards.
        ...
        try:
            ...
            try:
                ...
                pass
        for setter in ('setRelinearizeSkip', 'relinearizeSkip'):
            try:
                if setter.startswith('set'):
                    getattr(isam_params, setter)(1)
                else:
                    setattr(isam_params, setter, 1)
                break
